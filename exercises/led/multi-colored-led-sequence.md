### Purpose:
The RGB LED setup remains exactly the same as the previous exercise (rbg-led). However, this time we will add a potentiometer by connecting one outer pin to 5V, the other to GND, and its center pin (the wiper) to an Arduino analog input pin.
